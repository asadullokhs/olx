    ...
    try {
      ...
      if (req.files) {
        ...
        if (image.size > 1000000) {
          return res
            .status(403)
            .json({ message: "Image size must be less than (1) MB" });
        }
        const messageImage = await cloudinary.v2.uploader.upload(
          image.tempFilePath,
          {
            folder: "Olx",
          },
      ...
      if (updateMessage) {
        if (req.files) {
          ...
          if (image) {
            ...
            const messageImagee = await cloudinary.v2.uploader.upload(
              image.tempFilePath,
              {
                folder: "Olx",
              },
            ...
            const messageImag = {
              public_id: messageImagee.public_id,
              url: messageImagee.secure_url,
            };
            req.body.file = messageImag;
          }
        }
        const isMessage = await Message.findByIdAndUpdate(messageId, req.body, {
          new: true,
        });
        return res
          .status(200)
          .json({ message: "Message update successfully", isMessage });
      }
      res.status(404).json({ message: "Message not found" });
    } catch (error) {
      res.status(503).json({ message: error.message });
